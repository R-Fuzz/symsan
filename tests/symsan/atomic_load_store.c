...
int main (int argc, char** argv) {
  ...
  // Round-trip the symbolic value through an atomic store and an atomic load.
  // With taint-preserve-atomic-shadow (default) the shadow must survive both,
  // keeping the branch below symbolic.
  uint32_t a = 0;
  __atomic_store_n(&a, x, __ATOMIC_SEQ_CST);
  uint32_t v = __atomic_load_n(&a, __ATOMIC_SEQ_CST);
