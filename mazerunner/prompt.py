    ...
    def build_concret_divergent_branch_prompt(self, episode, divergent_branch_info, input_content):
        ...
        for filepath in related_code_info:
            ...
            for _, fn_guid in functions:
                prompt += self._func_codes[fn_guid] + '\n......\n'
        prompt += "<Source Code Ends>\n"
        
        # prompt building with input content
        prompt += "\n"
        prompt += "<Input Content Begins>\n"
        prompt += utils.hexdump(input_content) + '\n'
        prompt += "<Input Content Ends>\n"
        
        # prompt building with trace info
        prompt += "\n"
        prompt += "<Trace Info Begins>\n"
        prompt += trace_str
        prompt += "<Trace Info Ends>\n"

        # prompt building with concrete divergent branch
        prompt += "\n"
        prompt += "<Concrete Divergent Branch Info Begins>\n"
        prompt += divergent_branch_info_str + '\n'
        prompt += "<Concrete Divergent Branch Info Ends>\n"
        
        # prompt building with questions
        prompt += PromptBuilder.CONCRET_DIVERGENT_BRANCH_PROMPT
        return prompt
