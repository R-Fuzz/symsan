...
# here instead: this is the repo whose history explains a difference in the
# numbers, even though fetch.sh clones symsan fresh from GitHub for every build
# (see --branch) rather than shipping anything from this checkout.
#
# What it does that `./tools/captain/run.sh <rc>` on its own does not:
#
#   1. *Deletes the old image first*.  captain's run.sh skips the build when an
#      image of the same name exists, so without the rmi a campaign silently
#      measures whatever was built last week -- or last time this branch had
#      different content on GitHub.  That is the single most expensive mistake
#      available here, because nothing in the output says which build ran.
#   2. Records what went into the image (the branch and its resolved commit,
#      via --branch), and refuses --no-rebuild when the request no longer
#      matches it.
...
options:
  --arm ARM         both | symsan | cmplog | havoc   (default: both)
                      ...
                      cmplog  cmplog only; USE_SYMSAN=0, own build, branch map
                              forced off.  The concolic stage is not built and
                              not loaded -- not merely left with no solvers.
                      havoc   neither stage; USE_SYMSAN=0 USE_CMPLOG=0, own
                              build, branch map forced off.
                    cmplog and havoc are *LibAFL* baselines, not stock AFL++ --
                    they hold the scheduler, mutators, map and harness fixed and
                    vary only the stage, so they answer "does the concolic stage
                    earn its keep here", not "do we beat AFL++".  Both drop the
                    branch map, so their edge ids come from a per-TU AFL build
                    and are NOT the two-stage ids the symsan arms use: compare
                    their corpora through one common coverage binary, never by
                    subtracting two shared_mem counts.
  ...
  --branch B        branch of github.com/r-fuzz/symsan for fetch.sh to clone
                    inside the image (default: main).  There is no local-
                    checkout option any more: fetch.sh always clones, so
                    testing an unpushed change means pushing it to a branch
                    first and passing --branch.
...
# This checkout's own root -- used only to point the trailing hints at
# tools/magma/results.sh and extract-corpus.sh.  It plays no part in what
# fetch.sh ships any more; that always comes from a fresh clone (see BRANCH).
...
# The two baselines are coverage builds and nothing else, so they turn the
# branch map off rather than making the caller remember to -- the map's only
# consumer is the concolic stage these arms do not build, and instrument.sh
# refuses the combination outright.  An explicit --branch-map 1 is still an
# error: it says the caller believes something about this arm that is not true.
if [ "$ARM" = havoc ] || [ "$ARM" = cmplog ]; then
    if [ "$BRANCH_MAP_SET" = 1 ] && [ "$BRANCH_MAP" = 1 ]; then
        echo "--arm $ARM builds no concolic target, so there is nothing for" \
             "--branch-map 1 to feed." >&2
        exit 2
    fi
    if [ "$SOLVE_UB" = 1 ]; then
        echo "--arm $ARM runs no concolic stage, so --solve-ub would be" \
...
instrumentrc_body() {
    ...
    # Read by fetch.sh, not instrument.sh -- see the note there.  Always
    # emitted (not gated on differing from fetch.sh's own default) so that a
    # --branch change is visible in WANT_RC and trips the --no-rebuild
    # mismatch check below like any other build option.
    echo "SYMSAN_BRANCH=$BRANCH"
    [ "$BRANCH_MAP" = 1 ] && echo "USE_BRANCH_MAP=1"
    [ "$ARM" = symsan ]   && echo "USE_CMPLOG=0"
    # USE_SYMSAN=0 also plants $OUT/no_symsan, which is what lets run.sh tell
    # "this arm has no concolic stage" apart from "the concolic build failed" --
    # it refuses to start on the latter.
    #
    # cmplog turns the concolic stage off at *build* time, not with the fuzzer's
    # --symsan-no-i2s --symsan-no-jigsaw.  Those only empty the solver list: the
    # stage still loads, still forks the instrumented binary, still traces every
    # execution and still builds a task per branch, and then throws all of it
    # away.  That measures what tracing costs, which is a fair question but not
    # this one -- as a baseline it charges the fuzzer for work no baseline does.
    # (Measured on xmllint: 15 exec/s against the both arm's 278, with 249k
    # tasks built and zero solved.  If you want that control back, it is
    # `--arm both --fuzzargs "--symsan-no-i2s --symsan-no-jigsaw"`.)
    [ "$ARM" = cmplog ]   && echo "USE_SYMSAN=0"
    # Both off for havoc.
...
# Fingerprint of the source that goes into the image.  fetch.sh always clones
# github.com/r-fuzz/symsan fresh (see SYMSAN_BRANCH above), so what determines
# the content is that remote branch's current head, not anything in this local
# checkout -- resolved here, on the host, purely so the stamp can tell whether
# $BRANCH has moved since the last build.
#
# `|| true`: a network hiccup here shouldn't abort the whole script under
# set -e; it degrades to an empty fingerprint, which --no-rebuild then reports
# as a mismatch rather than trusting a check that never ran.
symsan_fingerprint() {
    git ls-remote "https://github.com/r-fuzz/symsan.git" "$BRANCH" 2>/dev/null \
        | cut -f1 | head -1 || true
}
...
echo "== symsan   github.com/r-fuzz/symsan @ $BRANCH${FINGERPRINT:+ ($( echo "$FINGERPRINT" | cut -c1-10))}"
echo "== magma    $MAGMA"
...
if [ "$REBUILD" = 1 ]; then
    ...
    # Reusing an image is fine -- --solve-ub and --fuzzargs vary the run and not
    # the build, so those arms share one -- but only if it is the build being
    # asked for.  Every named arm now has its own instrumentrc, so none of them
    # can reuse another's image; the stamp check below is what says so.
    for t in "${TARGETS[@]}"; do
...
# Nothing arm-specific here any more: every arm is now a build, so run.sh picks
# the stages up from what is in $OUT and needs no flag to say which arm it is.
FUZZARGS=""
